        ...
        if (mc.player.isCreative()) {
            ...
            if (handItem.getItem() != Items.AIR) {
                if (!mc.player.getMainHandStack().getOrCreateTag()
                    ...
                    if (items == 0) {
                        CodeUtilities
                            .chat("There are no items stored in this container!", ChatType.FAIL);
                    } else {
											if (items == 1) {
												CodeUtilities.chat("Unpacked §b" + items + "§a item!");
											} else {
												CodeUtilities
													.chat("§aUnpacked §b" + items + "§a items!", ChatType.SUCCESS);
											}
                    }
                } else {
