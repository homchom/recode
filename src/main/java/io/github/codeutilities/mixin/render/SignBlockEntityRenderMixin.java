...
public class SignBlockEntityRenderMixin {
    ...
    public void render(SignBlockEntity signBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
        if (!signBlockEntity.getPos().isWithinDistance(mc.cameraEntity.getBlockPos(), ModConfig.getConfig().signRenderDistance)) return;

        TextRenderer textRenderer = mc.textRenderer;

        if(FuncSearchUtil.shouldGlow(signBlockEntity) && DFInfo.currentState == DFInfo.State.DEV && ModConfig.getConfig().functionProcessSearch) {
