    ...
    public void register(Minecraft mc, CommandDispatcher<FabricClientCommandSource> cd) {
        cd.register(ArgBuilder.literal("imageparticle")
                .then(ArgBuilder.literal("load")
                        .then(ArgBuilder.argument("filename", PathArgumentType.folder(ExternalFile.IMAGE_FILES.getPath(), true))
                                .executes(ctx -> {
                                    try {
                                        File f = PathArgumentType.getPath(ctx, "filename").toFile();
                                        ...
                                        if (f.exists()) {
                                            ...
                                            TemplateUtils.compressTemplateNBT(stack, fileName, mc.player.getName().getContents(), convertToTemplate(image, fileName));
                                            stack.setHoverName(new TextComponent("§6IMAGE§7 -§f " + fileName));
                ...
                .then(ArgBuilder.literal("printer")
                        .executes(ctx -> {
                            try {
                                ...
                                stack.setHoverName(new TextComponent("§b§lFunction §3» §bRecode§6 Particle Image Printer"));
