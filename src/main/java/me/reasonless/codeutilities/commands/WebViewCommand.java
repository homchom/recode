...
public class WebViewCommand {
	public static int execute(MinecraftClient mc, CommandContext<CottonClientCommandSource> ctx) {
		...
		try {
			...
			text.getStyle().setClickEvent(new ClickEvent(ClickEvent.Action.byName("field_11749"), String.format("https://derpystuff.gitlab.io/code/?template=%s", templateJson.code)));
