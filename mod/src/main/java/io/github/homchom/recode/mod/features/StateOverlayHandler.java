...
public class StateOverlayHandler {
    ...
    public static void drawStateOverlay(Font tr, GuiGraphics guiGraphics) {
        if (DF.isOnDF()) {
            if (state != null) {
                if (state instanceof DFState.OnPlot playState) {
                    ...
                    drawTextRight(Component.literal("You are currently " + playState.getMode().getId().getCapitalizedDescriptor()).withStyle(style -> style.withColor(TextColor.fromLegacyFormat(ChatFormatting.DARK_AQUA))).getVisualOrderText(), 32, tr, guiGraphics);
